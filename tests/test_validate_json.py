...
class TestRouteValidateJson(FlaskBaseTestCase):
    def test_app_deve_conter_a_rota_validate_json(self):
        ...
        self.assertIn('validate_json', endpoints,
                      'o endpoint validate-json não existe no APP')
    ...
    def test_deve_receber_um_objeto_json(self):
        response = self.client.post(url_for('validate_json'), json={})
        self.assertIsInstance(response.json, dict)

    def test_deve_receber_um_objeto_vazio_e_retornar_o_schema(self):
        response = self.client.post(url_for('validate_json'), json={})
        schema_esperado = {
            "address": [
    ...
    def test_deve_retornar_dados_validos_com_json_correto(self):
        response = self.client.post(url_for('validate_json'), json=json_valido)
